...
==== Example of roles
* User: Writes the fixture. He therefore describes in a record or table what the input parameters are and what the result must be.
* Tester: Is responsible for writing the tests. They get their input values and the results via annotations. The test cases are then implemented to verify that the implementation is correct.
* Developer: Implements the required functionality.

=== Supported Platforms
As for now, only working with local files and Confluence is supported.
